...
void protoss_build_nexus(char* f_name,partition_t* nexus,int pflag,int cflag){
  ...
  if(!cflag){ 
    fputs("\n",fp);
    fputs("charpartition ",fp);
    fputs(nexus->partition_names[0],fp);
    part_name_check = malloc (sizeof(char) * (strlen(nexus->partition_names[0])+1));
    strcpy(part_name_check,nexus->partition_names[0]);
    fputs(" =\n",fp);
  }
  for(i=0;i<nexus->part_len;i++){
    if(!cflag && strcmp(part_name_check,nexus->partition_names[i])){
      free(part_name_check);
      part_name_check = malloc (sizeof(char) * (strlen(nexus->partition_names[i])+1));
      strcpy(part_name_check,nexus->partition_names[i]);
      fseek(fp, -2L, SEEK_CUR);
      fputs(";\n\n",fp);
      fputs("charpartition ",fp);
      fputs(nexus->partition_names[i],fp);
      fputs(" =\n",fp);
    }
    if(cflag){
      fputs(nexus->partition_names[i],fp);
    ...
    if(!pflag)
      ...
      fputs(nexus->parameter_names[i],fp);
    if(cflag)
      fputs("\t",fp);
    else
      fputs(": ",fp);
  ...
  if(!cflag)
    fputs("end;",fp);
  fclose(fp);
  if(part_name_check != NULL) free(part_name_check);
}
...
partition_t parse_partition(int argc,char** argv){
  ...
  /******************************************/
  /* Variables for reading input parameters */
  FILE *datei = NULL;  /* fp for partition file in NEXUS */
  int cflag=0;  /* csv output if set */
  int all_flag=0;      /* flag whether all partition lines shoul be concatenated [OFF] */
  int paras_flag=0;    /* flag whether to parse evol. model params [OFF] */
  char output[80];
  /******************************************/
  ...
  output[0] = '\0';
/*Flags....
  a....if more then one partition is specified
  p....output model parameters if set
  h....show help
  c....CSV instead of a NEXUS File as output
  o....Specify output file name
  */
  while ((i = getopt (argc, argv, "hapco:")) != -1)
    switch (i) {
      case 'a':
        all_flag = 1;
        break;
...
/* Open File and reports error if not enough or good parameters */  
  if(output[0]=='\0')
    strcpy(output,"hugo");
  if(argc<2){
...
/*        fseek(datei, -1L, SEEK_CUR); */
    ...
    if(charpart_flag == 0 && c=='c') charpart_flag++;
      ...
      if(all_flag!=1)current_pos =0;
      temp = 0;
    /*  fseek(datei, -1L, SEEK_CUR); */
      name_right = grepName(c,datei,vname,str,str_len,&c_read);
      /* check if 'all_flag' is set */
      if(all_flag==1){
        vname[0] = 'a';
        vname[1] = 'l';
        vname[2] = 'l';
...
/* fprintf(stderr,"ERROR %s:%d\n",__FILE__,__LINE__); */
  ...
  free(pset_st);
  free(pset_leng);
  protoss_build_nexus(output,&ret,paras_flag,cflag);
  return ret;
} /* parse_partition */
